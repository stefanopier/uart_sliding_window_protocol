...
static void send_packet_struct(Packet *pkt) {
    ...
    // Indicate an outbound packet was sent (code 3 = packet sent)
    swp_debug_pulse(3);
...
static bool receive_any_frame(FrameType *type, Packet *pkt, uint8_t *ctrl_data, uint8_t *ctrl_len) {
    ...
    *type = FRAME_TYPE_DATA;
    ...
    while (flen < sizeof(frame)) {
        ...
        byte = uart_receive_byte();
        last_byte_ms = millis();
        if (in_escape) { frame[flen++] = byte ^ STUFF_BYTE; in_escape = false; }
        else if (byte == ESCAPE_CHAR) { in_escape = true; }
        else if (byte == FRAME_BYTE) { break; }
        else { frame[flen++] = byte; }
    }
    ...
    // Extract data
    for (uint16_t i = 0; i < pkt->data_length; i++) pkt->data[i] = frame[12 + i];
    // Extract token (LE)
...
static uint8_t generate_sack_bitmap(void) {
    uint8_t bitmap = 0;
    for (int i = 0; i < WINDOW_SIZE; i++) {
        if (recv_window[i].received) bitmap |= (1 << i);
    }
    return bitmap;
}
...
static void process_in_order_packets(uint8_t *output, uint16_t *output_len) {
    while (recv_window[0].received && recv_window[0].pkt.seq == expected_seq) {
        ...
        *output_len += p->data_length;
        if (p->flags & FLAG_LAST_PACKET) break;
...
static void handle_reset_frame(void) {
    ...
    base_seq = 0; next_seq = 0; expected_seq = 0;
